...
export type SaveBatchResult = { commitSha: string; changed: number };

/**
 * Multi-file (single-commit) write used by i18n saves and the scheduling
 * engine. Mirrors the files-batch route's commit logic.
 */
export async function saveBatchCore(input: SaveBatchInput): Promise<SaveBatchResult> {
  const { owner, repo, branch, name, token, config } = input;
  ...
  const normalizedStrictPaths = Array.isArray(input.strictPaths)
    ? new Set(input.strictPaths.map((p: string) => normalizePath(p)))
    : null;
  ...
  const newCommitRes = await octokit.rest.git.createCommit({
    owner,
    repo,
    message: input.message || `Reorder ${name} (via Pages CMS)`,
  ...
  await Promise.all([
    ...blobs.map((blob) =>
      updateFileCache("collection", owner, repo, branch, {
        type: "modify",
        path: blob.path,
        sha: blob.blobSha,
        content: blob.text,
  ...
  return { commitSha: newCommitRes.data.sha, changed: blobs.length + normalizedDeletions.length };
}
